...
function buildJSONSandbox(payload: string): string {
  const escaped = payload.replace(/`/g, "\\`").replace(/<\/script>/g, "<\\/script>");
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8"/>
  <meta name="viewport" content="width=device-width,initial-scale=1"/>
  <style>
    body{margin:0;background:#0b0b0b;color:#e5e5e5;font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,monospace;padding:20px}
    pre{margin:0;white-space:pre-wrap;line-height:1.6;font-size:13px}
  </style>
</head>
<body>
  <pre id="json"></pre>
  <script>
    ...
    try {
      ...
      node.textContent = JSON.stringify(parsed, null, 2);
    } catch {
...
const PREVIEWABLE_TYPES = new Set(["react", "html", "svg", "markdown", "csv", "mermaid", "json"]);
...
export default function CanvasPanel({ artifact, onClose }: CanvasPanelProps) {
  ...
  const getSandboxHTML = useCallback((): string => {
    ...
    switch (artifact.type) {
      ...
      case "json":     return buildJSONSandbox(artifact.content);
      default:
