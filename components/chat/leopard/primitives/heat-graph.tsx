...
// Leopard amber ramp (dark-tuned): empty hairline → deep amber → solid amber.
const COLORS = ["rgba(128,128,128,0.12)", "#5a4306", "#8f6d0a", "#d49600", "#ffb400"];
  ...
  return (
    <div className="relative ms-10 h-5">
      <HeatGraphPrimitive.MonthLabels>
        {({ label, totalWeeks }) => (
          <span className="absolute text-xs text-foreground/40" style={{ left: `${(label.column / totalWeeks) * 100}%` }}>
  ...
  return (
    <div className="flex w-8 shrink-0 flex-col justify-between py-[2px]">
      <HeatGraphPrimitive.DayLabels>
        {({ label }) => (
          <span className="flex h-[13px] items-center text-xs text-foreground/40">
...
function CellTooltip() {
  return (
    <HeatGraphPrimitive.Tooltip className="pointer-events-none rounded-md bg-foreground px-3 py-1.5 text-xs whitespace-nowrap text-background">
      {({ cell }) => (
        <>
...
function GraphLegend() {
  return (
    <div className="ms-auto flex items-center gap-1 text-xs text-foreground/40">
      <span>Less</span>
