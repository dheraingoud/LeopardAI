...
// Measures child elements by data-flow-id and draws routed SVG edges between
// them. Verbatim logic from upstream; only class names were re-tokened.
  ...
  const measure = useCallback(() => {
    ...
    for (const edge of edges) {
      ...
      if (route === "down") {
        ...
        const midY = from.bottom + (to.top - from.bottom) * (edge.midFrac ?? 0.5);
        const laneX = edge.laneOffset === undefined ? undefined : sx + edge.laneOffset;
        ...
        const sx = from.cx + (edge.fromOffset ?? 0);
        const ex = to.cx + (edge.toOffset ?? 0);
        const laneY = Math.max(from.bottom, to.bottom) + (edge.laneOffset ?? 28);
  ...
  return (
    ...
    >
      <svg
        ...
        className="pointer-events-none absolute inset-0 z-0 h-full w-full overflow-visible text-foreground/30"
      >
        {rendered.map((edge, index) => (
          <g key={index}>
            <path d={edge.path} fill="none" stroke="currentColor" strokeWidth={1.5} />
        ...
        (edge, index) =>
          edge.label && (
            <span
              ...
              className="absolute z-20 -translate-x-1/2 -translate-y-1/2 bg-background px-1.5 text-xs whitespace-nowrap text-foreground/50"
