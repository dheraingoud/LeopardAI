...
export function FlowGraph({
  ...
  return (
    <div
      ...
      className={cn(paper, "w-full max-w-md overflow-x-auto rounded-lg p-4", className)}
      {...props}
    >
      <div className="relative" style={{ width, height, minWidth: width }}>
        ...
        {shown.map((node) => (
          <div
            key={node.id}
            className={cn(
              "fade-in zoom-in-95 animate-in fill-mode-both absolute flex items-center justify-center rounded-md border text-center text-[11.5px] leading-tight duration-300",
              node.state === "done" &&
                "border-foreground/10 bg-foreground/[0.04] text-foreground/50",
              node.state === "active" &&
                "text-foreground/90 dark:border-[#ffb400]/30 dark:bg-[#ffb400]/10 light:border-[#d49600]/30 light:bg-[#d49600]/10",
