...
export function ChatPanel({ className, ...props }: ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        paper,
        "flex h-[270px] w-full max-w-md flex-col overflow-hidden rounded-lg",
        className,
      )}
      {...props}
    />
    ...
    >
      {["-0.32s", "-0.16s", "0s"].map((delay) => (
        <span
          key={delay}
          aria-hidden
          className="size-1 animate-bounce rounded-full motion-reduce:animate-none dark:bg-[#ffb400]/60 light:bg-[#d49600]/70"
