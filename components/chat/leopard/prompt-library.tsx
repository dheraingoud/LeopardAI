...
> & {
  ...
  return (
    <div
      ...
      className={cn(paper, "flex w-full max-w-sm flex-col gap-2 rounded-lg p-3", className)}
      {...props}
    >
      <div className={cn(field, "flex items-center gap-2 rounded-md px-2.5 py-1.5")}>
      ...
      <div id={listId} role="listbox" aria-label="Saved prompts" className="flex flex-col">
        {matches.map((prompt) => (
          <button
            ...
            className={cn(
              "flex items-center gap-2 rounded-md px-2 py-1.5 text-start transition-colors",
      ...
      {selected && (
        <div
          className={cn(
            field,
            "fade-in animate-in flex flex-col gap-2 rounded-md p-2.5 duration-200",
          )}
        >
            ...
            <div className="flex flex-wrap gap-1">
              {selected.variables.map((variable) => (
                <span
                  ...
                  className={cn(
                    mono,
                    "bg-background/70 rounded px-1.5 py-0.5 dark:text-[#ffb400]/60 light:text-[#d49600]/70",
                  )}
                >
                  {`{${variable}}`}
                </span>
