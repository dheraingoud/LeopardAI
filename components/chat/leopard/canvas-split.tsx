...
export function CanvasSplit({ className, ...props }: ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        paper,
        "flex w-full max-w-3xl flex-col overflow-hidden rounded-lg md:h-80 md:flex-row",
        className,
      )}
      {...props}
    />
  ...
  return (
    <div
      ...
      className={cn(
        ...
        speaker === "user"
          ? cn(field, "text-foreground/80 ms-auto rounded-lg px-3 py-2")
          : "text-foreground/60",
        className,
      )}
      {...props}
    ...
    >
      ...
      <span
        className={cn(
          ...
          saved ? "dark:text-[#ffb400] light:text-[#d49600]" : "text-foreground/30",
        )}
      >
        {saved ? (
      ...
      <button
        ...
        onClick={onCopy}
        disabled={!onCopy}
        className={cn(ghostButton, "size-7 shrink-0 disabled:pointer-events-none disabled:opacity-30")}
      >
  ...
  return (
    ...
    >
      ...
      {writing && (
        <span
          aria-hidden
          className="h-[1.05em] w-[2px] animate-pulse rounded-full motion-reduce:animate-none dark:bg-[#ffb400]/80 light:bg-[#d49600]/80"
        />
      )}
    </div>
  );
