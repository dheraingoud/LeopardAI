    ...
    >
      {open && (
        <div
          className={cn(
            floating,
            "fade-in zoom-in-95 slide-in-from-bottom-2 animate-in flex w-full flex-col gap-3 rounded-lg p-4 duration-200",
          )}
        >
          ...
          <div className="flex flex-col gap-1.5">
            {prompts.map((prompt) => (
              <button
                ...
                className={cn(
                  field,
                  "hover:bg-foreground/[0.07] text-foreground/70 rounded-md px-3 py-2 text-start text-[13px] transition-colors",
                )}
              >
                {prompt}
              </button>
