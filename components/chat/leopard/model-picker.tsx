...
export function ModelPicker({
  ...
  return (
    <div
      ...
      className={cn(paper, "flex w-full max-w-sm flex-col gap-1 rounded-lg p-2", className)}
      {...props}
    >
      {families.map((family) => (
            ...
            .map((model) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "flex items-center gap-2.5 rounded-md px-2 py-2 text-start transition-colors",
                    selected ? "bg-foreground/[0.06]" : "hover:bg-foreground/[0.035]",
                  )}
                >
                  <span className="flex size-3.5 shrink-0 items-center justify-center">
                    {selected && (
                      <CheckIcon className="fade-in zoom-in-90 animate-in size-3.5 duration-200 dark:text-[#ffb400] light:text-[#d49600]" />
                    )}
                  </span>
